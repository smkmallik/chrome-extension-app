...
const Todo = () => {
    ...
    const addTodo = () => {
        todoList.findIndex((item) => item.todo === input) === -1 && setTodoList((x) => [...x, {
            todo: input,
            checked: false
        }]);
        setInput("");
    ...
    const toggleChecked = (x) => {
        setTodoList((todoList) => todoList.map((curr) => curr.todo === x.todo ? {
            ...curr,
    ...
    const deleteCheckedTodo = () => {
        setTodoList((todoList) => todoList.filter((item) => item.checked === false));
    }
